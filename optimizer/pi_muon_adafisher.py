...
class PI_Muon_AdaFisher(AdaFisherBackBone):
    ...
    @torch.no_grad()
    def step(self, closure: None | CallableABC[[], torch.Tensor] = None, effective_gamma: float | None = None):
        if closure is not None:
            raise NotImplementedError("Closure not supported for PI-Muon-AdaFisher.")
        
        # 使用 effective_gamma 计算 pi_value，类似于 F3EPI
        pi_value = 0.5  # 默认值
        ...
        for group in self.param_groups:
            ...
            for _ in range(len(self.modules)):
                ...
                muon_grad = muon_update(
                    grad,
                    state['muon_momentum_buffer'],
                    beta=self.muon_momentum
                )
                muon_grad = muon_grad.reshape(grad.shape)
                ...
                if self._check_dim(param_list, idx_module, idx_param):
                    ...
                    if isinstance(F_tilde, list):
                        ...
                        g_update_w = (1 - lambda_t) * g_fisher_w + lambda_t * muon_grad
                        ...
                        if idx_param < len(param_list) and param_list[idx_param].grad is not None:
                            ...
                            g_update_b = (1 - lambda_t) * g_fisher_b + lambda_t * muon_grad
                            self._apply_adamw_update(group, p_bias, g_update_b)
                            idx_param += 1
                    else:
                        g_fisher = grad / (F_tilde + self.Lambda)
                        g_update = (1 - lambda_t) * g_fisher + lambda_t * muon_grad
                        self._apply_adamw_update(group, p, g_update)
                        idx_param += 1
                    
                    idx_module += 1
                else:
                    g_update = lambda_t * muon_grad
                    self._apply_adamw_update(group, p, g_update)
                    idx_param += 1
