...
**IPWT** 是整个研究计划的理论与哲学基石，一个统一、计算上可行且具有强大解释力的意识科学理论框架。其核心方法论是对当代意识科学的三大主流理论——整合信息理论 (IIT)、全局工作空间理论 (GWT) 和预测编码/自由能原理 (PCT/FEP)——进行的功能驱动的重构与创造性融合。
...
- **不准确性成本 (Inaccuracy)**: 对应于神经网络**精度加权的损失函数值 (precision-weighted loss)**。这里的“精度”即模型对其预测的不确定度，由输出分布的香农熵 `τ` 度量。在交叉熵损失环境下，由于 `τ` 与 `L_CE` 数值上高度相关，可直接取输出熵 `τ` 作为 `PI` 的 `Accuracy` 度量。
